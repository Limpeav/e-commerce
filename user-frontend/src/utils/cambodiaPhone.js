...
export const isValidCambodiaMobilePhone = (phone = "") => {
  const localDigits = toCambodiaLocalPhoneDigits(phone);
  const prefix = localDigits.slice(0, 2);

  if (SEVEN_DIGIT_MOBILE_PREFIXES.has(prefix)) {
    return localDigits.length === 9;
  }

  return STANDARD_MOBILE_PREFIXES.has(prefix) && localDigits.length === 8;
};

export const normalizeCambodiaMobilePhone = (phone = "") => {
  const localDigits = toCambodiaLocalPhoneDigits(phone);
  return isValidCambodiaMobilePhone(localDigits) ? `+855${localDigits}` : null;
};
