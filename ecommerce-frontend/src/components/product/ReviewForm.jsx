import React from 'react';
import { Star, Check } from 'lucide-react';
import { useDarkMode } from '../../hooks';
import { useLanguage } from '../../context/useLanguage';

const ReviewForm = ({ reviewData, onSubmit }) => {
  const [isDark] = useDarkMode();
  const { t } = useLanguage();
  const {
    rating,
    setRating,
    comment,
    setComment,
    submittingReview,
    reviewError,
    alreadyReviewed
  } = reviewData;

  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <h3 className="text-sm font-black text-text-main uppercase tracking-widest mb-6">{t("product.writeReview")}</h3>
      
      {alreadyReviewed ? (
        <div className={`p-6 rounded-[2rem] text-sm border ${isDark ? "bg-emerald-500/10 text-emerald-300 border-emerald-500/20" : "bg-green-50 text-green-700 border-green-100"}`}>
          <div className="flex items-center gap-3 mb-2">
            <Check className="w-5 h-5 text-green-600 font-black" />
            <span className="font-black uppercase tracking-widest">{t("product.reviewSubmitted")}</span>
          </div>
          <p className="font-bold opacity-80">{t("product.feedbackThanks")}</p>
        </div>
      ) : reviewError ? (
        <div className={`p-4 rounded-2xl text-xs font-bold border ${isDark ? "bg-red-500/10 text-red-300 border-red-500/20" : "bg-red-50 text-red-600 border-red-100"}`}>
          {reviewError}
        </div>
      ) : null}

      {!alreadyReviewed && (
        <>
          <div>
            <label className="block text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3 ml-1">{t("product.rating")}</label>
            <select
              value={rating}
              onChange={(e) => setRating(e.target.value)}
              className={`w-full border-2 rounded-2xl p-4 focus:outline-none focus:border-primary transition-all font-black text-text-main ${isDark ? "bg-slate-800 border-slate-700" : "bg-stone-50 border-stone-100"}`}
            >
              <option value="" disabled>{t("product.selectRating")}</option>
              <option value="5">5 - {t("product.excellent")}</option>
              <option value="4">4 - {t("product.veryGood")}</option>
              <option value="3">3 - {t("product.good")}</option>
              <option value="2">2 - {t("product.poor")}</option>
              <option value="1">1 - {t("product.bad")}</option>
            </select>
          </div>
          
          <div>
            <label className="block text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3 ml-1">{t("product.yourReview")}</label>
            <textarea
              rows="4"
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              placeholder={t("product.reviewPlaceholder")}
              className={`w-full border-2 rounded-2xl p-5 focus:outline-none focus:border-primary transition-all text-text-main font-bold placeholder:font-normal ${isDark ? "bg-slate-800 border-slate-700 placeholder:text-slate-500" : "bg-stone-50 border-stone-100"}`}
            ></textarea>
          </div>
          
          <button
            type="submit"
            disabled={submittingReview}
            className="w-full bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-indigo-700 transition-all transform active:scale-95 disabled:opacity-50 uppercase tracking-[0.2em] text-[10px]"
          >
            {submittingReview ? t("product.submittingReview") : t("product.submitReview")}
          </button>
        </>
      )}
    </form>
  );
};

export default ReviewForm;
