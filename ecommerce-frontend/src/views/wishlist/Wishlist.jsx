      ...
      <div className="min-h-screen bg-bg-base flex items-center justify-center py-20 px-6 font-sans">
        <div className="text-center max-w-lg mx-auto bg-white p-16 rounded-[4rem] border border-stone-100 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative mb-10">
            <div className="bg-stone-50 w-32 h-32 rounded-[2.5rem] flex items-center justify-center mx-auto shadow-inner border border-stone-100">
              <Heart className="w-14 h-14 text-primary/20" />
            </div>
          </div>

          <h2 className="text-4xl md:text-5xl font-bold text-text-main mb-6 font-display tracking-tight leading-tight">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {wishlist.map((product) => (
            <div
              key={product._id}
              className="relative bg-white rounded-[2rem] border border-stone-100 p-4 hover:shadow-lg transition-all duration-300 group flex flex-col"
            >
              {/* Product Image */}
              <Link to={`/products/${product._id}`} className="block relative bg-stone-50 rounded-[1.5rem] p-6 aspect-square mb-4 overflow-hidden">
                <div className="absolute top-3 right-3 z-20">
                  <button
                    ...
                    className="bg-white/90 backdrop-blur-sm p-2 rounded-xl shadow-sm border border-white hover:bg-red-50 hover:text-red-500 transition-all text-stone-300 transform group-hover:scale-105"
              ...
              <div className="px-2 flex-1 flex flex-col">
                <div className="mb-4 flex-1">
                  <Link to={`/products/${product._id}`}>
                    <h3 className="font-bold text-text-main text-lg mb-2 line-clamp-1 hover:text-primary transition-colors tracking-tight">
                      {product.title}
                    </h3>
                  </Link>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-stone-400 font-bold uppercase tracking-wide mb-1">Price</span>
                    <span className="text-xl font-bold text-text-main tracking-tight">
                      ${product.price ? product.price.toFixed(2) : "0.00"}
                    </span>
                  </div>
                  <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${product.stock > 0
