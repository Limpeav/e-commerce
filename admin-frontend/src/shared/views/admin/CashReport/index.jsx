    ...
    const statCards = [
        {
            ...
            value: (
              <div className="flex items-baseline gap-2 flex-wrap">
                <Price amount={summary.totalCash} showKHR={false} />
                {summary.totalCashKHR > 0 && (
                  <span className="text-sm font-bold text-green-600/60">
                    (≈ {Math.round(summary.totalCashKHR).toLocaleString()}៛)
                  </span>
                )}
              </div>
            ),
                                            ...
                                            <tr key={day.date} className="hover:bg-gray-50">
                                                ...
                                                <td className="whitespace-nowrap px-5 py-4 text-right font-black text-gray-950">
                                                    <div className="flex flex-col items-end">
                                                        <Price amount={day.totalCash} usdClassName="text-gray-950" showKHR={false} />
                                                        {day.totalCashKHR > 0 && (
                                                            <span className="text-xs font-bold text-gray-500">
                                                                ≈ {Math.round(day.totalCashKHR).toLocaleString()}៛
                                                            </span>
                                                        )}
                                                    </div>
                                                </td>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {filteredOrders.map((order) => (
                                        ...
                                        >
                                            ...
                                            <td className="whitespace-nowrap px-5 py-4 text-right font-black text-gray-950">
                                                <Price amount={order.totalPrice} exchangeRate={order.exchangeRateAtOrder} usdClassName="text-gray-950" />
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
