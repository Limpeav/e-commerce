...
class _ProductReviewsPageState extends State<ProductReviewsPage> {
  ...
  DateTime? _parseReviewDate(String rawDate) {
    ...
    final dmy = RegExp(r'^(\d{1,2})[-/](\d{1,2})[-/](\d{4})$').firstMatch(trimmed);
    if (dmy != null) {
    ...
    // 3. Try "Month Day, Year" or "Month Day Year" (e.g. "Oct 20, 2020", "Sep 6, 2026")
    const monthNames = {
      'jan': 1, 'feb': 2, 'mar': 3, 'apr': 4, 'may': 5, 'jun': 6,
      'jul': 7, 'aug': 8, 'sep': 9, 'oct': 10, 'nov': 11, 'dec': 12,
    };
    final mdy = RegExp(r'^([a-zA-Z]+)\s+(\d{1,2}),?\s+(\d{4})$').firstMatch(trimmed);
    if (mdy != null) {
      final monStr = mdy.group(1)!.toLowerCase().substring(0, 3);
      final m = monthNames[monStr];
  ...
  String _formatDisplayDate(String rawDate) {
    final parsed = _parseReviewDate(rawDate);
    if (parsed != null) {
    ...
    // Fallback: If rawDate is like YYYY-MM-DD or YYYY/MM/DD
    final ymd = RegExp(r'^(\d{4})[-/](\d{1,2})[-/](\d{1,2})').firstMatch(rawDate.trim());
    if (ymd != null) {
      final y = ymd.group(1)!;
  ...
  // Newest comments on top of old comments
  List<ProductReview> get _sortedReviews {
    final indexed = _product.reviews.asMap().entries.toList();
    indexed.sort((e1, e2) {
      final date1 = _parseReviewDate(e1.value.date);
      final date2 = _parseReviewDate(e2.value.date);
      if (date1 != null && date2 != null) {
        final cmp = date2.compareTo(date1); // Newest date on top
        if (cmp != 0) return cmp;
      } else if (date1 != null) {
        return -1;
      } else if (date2 != null) {
      ...
      // If dates are identical or neither parsed, newer (higher original index) goes first
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Text(
                                    '${review.rating.toInt()} OUT OF 5, ${_formatDisplayDate(review.date)}',
                                    style: TextStyle(
                                      fontSize: 11,
                                      fontWeight: FontWeight.w500,
                                      color: isDark
