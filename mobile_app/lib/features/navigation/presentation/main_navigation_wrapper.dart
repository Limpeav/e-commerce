...
class _MainNavigationWrapperState extends State<MainNavigationWrapper> {
  ...
  bool _hasCheckedAppOpenReviews = false;
  ...
  void _checkPendingReviewsOnAppOpen() {
    if (!mounted || _hasCheckedAppOpenReviews) return;
    final authState = context.read<AuthBloc>().state;
    ...
    final hasDeliveredOrders = orderState.orders.any((o) =>
        o.status == OrderStatus.delivered || o.isDelivered || o.deliveredAt != null);

    if (hasDeliveredOrders && orderState.pendingReviewItems.isNotEmpty) {
      _hasCheckedAppOpenReviews = true;
      _showDeliveredReviewPrompt();
    }
  }
      ...
      listeners: [
        ...
        BlocListener<OrderBloc, OrderState>(
          listener: (context, orderState) {
            // Check once when orders finish loading on app open
            if (!_hasCheckedAppOpenReviews) {
              final authState = context.read<AuthBloc>().state;
              if (authState.status == AuthStatus.authenticated && authState.currentUser != null) {
                final hasDeliveredOrders = orderState.orders.any((o) =>
                    o.status == OrderStatus.delivered || o.isDelivered || o.deliveredAt != null);
